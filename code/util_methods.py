...
def check_wavespeedfield(c_field): # S.Tan added 04/01/2024
    """ Method checking and fixing wavespeed input to make sure it's xarray dataarray

    Args:
        c_field (float): 2d wavespeed field, first column must be c, second colume must be d

    Returns:
        c_field (float): 1d xarray DataArray object
    """
    logging.info('wavespeed field checker should ideally support xarray ds or da')

    if not type(c_field) == xa.DataArray:
        c_field = xa.DataArray(data=c_field[0,:],
                         dims=['d'],
                         coords=dict(
                                d=(['d'], c_field[1,:]),
                                )
