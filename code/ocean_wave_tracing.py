...
from util_methods import make_xarray_dataArray, to_xarray_ds, check_velocity_field, check_bathymetry, check_wavespeedfield
...
class Wave_tracing():
    ...
    def __init__(self, U, V,  nx, ny, nt, T, dx, dy,
        ...
        Args:
            ...
            c_field (float): 2D wave speed field 
            # S.Tan add to account for any given wave speed field, if not given, use linear gravity wave dispersion relation, 04/01/2024
            # c_field[2,:], first row is wave speed and second row is depth
            **kwargs
        """
        self.g = 9.81
        self.nx = nx
        ...
        # Check wavespeed field
        if c_field is not None:
            self.c_field = check_wavespeedfield(c_field=c_field)
        else:
            logging.info(f'Wavespeed field not given, use linear gravity wave dispersion relation to calculate wave group speed')
            self.c_field = None
    ...
    def c_intrinsic(self,k,d,group_velocity=False):
        ...
        Returns:
            instrinsic velocity (float): group or phase velocity depending on
                flag
        """
        if self.c_field is not None: # S.Tan change to calculate c_in from an input wavespeed field: c_field, 04/01/2024
            c_in = self.c_field.sel(d=d,method='nearest')
            n = 1
        else:
            g=self.g
            dw_criteria = k*d>25
            ...
            # g_prime = .005
            # c_in = np.sqrt(g_prime*d) # S.Tan change to a toy wave speed that is only a function of h, 03/27/2024, not gonna use it
    ...
    def sigma(self,k,d):
        ...
        sigma =  self.c_intrinsic(k,d)*k # S.Tan change to only accunt changes in cg(h), 04/01/2024
